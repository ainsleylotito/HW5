...
range(baton_rouge$reported_date)
```
...
### Need to cite and explain what this is doing ##
ba_subdivisions$layer <- "Subdivisions"
ba_tracts$layer       <- "Tracts"
ba_blocks$layer       <- "Blocks"

ggplot() +
  geom_sf(data = ba_subdivisions, 
          aes(color = layer), fill = NA, size = 0.7) +
  geom_sf(data = ba_tracts, 
          aes(color = layer), fill = "white", size = 0.3) +
  geom_sf(data = ba_blocks, 
          aes(color = layer), fill = NA, size = 0.1) +
  ...
  labs(title = "Baton Rouge, Louisiana",
       color = "Geography Layer") +
  theme_minimal()

```  


Now that I have a good basemap, I can look into adding some homicide info. 
...
ggplot() +
  ...
  geom_sf(data = baton_rouge_sf_top3, aes(color = race_group), size = 0.9, alpha = .6) +
